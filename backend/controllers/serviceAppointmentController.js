...
//parse the time and return AM : PM
...
function resolveClerkUserId(req) {
  try {
    // const auth = req.auth || {} //old ver
    // use the new req.auth() function
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    const services = await Service.aggregate([
      ...
      { $sort: { createdAt: -1 } },
    ]) //it will give total appointment, complete appt, cancel appt and total earning done by them
    return res.json({
      success: true,
      services,
